...
Which mode the key starts is picked in the settings screen. Every mode brings its own stance, its own set of headings and its own click pace, so the settings screen is the only place that needs touching to switch between them. Available modes:
...
The straight-line ninja bridge uses a 78-degree placement pitch. Its diagonal variant uses 76 degrees,
places two blocks from the exposed corner faces, walks backward without crouching, then engages crouch
again just before the next corner and repeats. Both cardinal and diagonal headings are supported.
